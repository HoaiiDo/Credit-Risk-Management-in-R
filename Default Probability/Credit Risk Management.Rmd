...
Keeping misisng data for `emp_length` and `int_rate` we are goona use coarse classification for this variables.
...
The NULL deviance is how well the reponse variable is predicted by model including only the intercept(grand mean).
Residual deviance includes independent vaariables. As you can see, an addition of 20 DOF resulting in a lowering of deviance by 584.
```{r}
...
Evaluating the logistic regression model on probability of defaulting with an example cut off of 0.15
...
Accurcy measures how correct the model identifies and exludes a given condition.
Sensibility measures how good the model is at detecting a true positive
Specificity measures how likely the trye negative are ruled out
...
auc(ROC_all_full)
```
The area under the curve of all full variables version of logistic regression is lowest. It is the most prerrable one so far.
...
auc(ROC_loss_matrix)
```
The area under the curve of the changed prior probabilities based decision tree is lowest. It is the most prerrable one so far.
